...
int main(void){
  ...
  double m0=1.0, R0=2.5,Vx0=0,Vy0=0,x0=10,y0=0;
  double  tmax=100;
  ...
  for(int jj = 0;jj<42;jj++){
	...
	tdibujo=0;
      ...
      */
      ...
      for(int ii = 0;ii<N;ii++)Particula[ii].Mueva_r(dt,ZETA);
      //x=Getx();
      if(Particula[0].Getx()>r_max){
	r_max=Particula[0].Getx();
      }
      if(Particula[0].Getx()<r_min){
	r_min=Particula[0].Getx();
      }
      
    }
    std::cout<<KbT<<"  "<<(r_max+r_min)*0.5<<std::endl;
