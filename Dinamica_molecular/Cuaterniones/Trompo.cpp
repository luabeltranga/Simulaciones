...
public:
  ...
  double getTheta(void){return std::acos((q0*q0-q1*q1-q2*q2+q3*q3)/(q0*q0+q1*q1+q2*q2+q3*q3));};
};
...
void CuerpoRigido::inicie(double Ix,double Iy,double Iz, double theta0,double phi0,double psi0,double wx0,double wy0,double wz0){
  ...
  //posicion inicial
  q0=std::cos(0.5*theta0)*std::cos(0.5*(phi0+psi0));
  q1=std::sin(0.5*theta0)*std::cos(0.5*(phi0-psi0));
  q2=std::sin(0.5*theta0)*std::sin(0.5*(phi0-psi0));
  q3=std::cos(0.5*theta0)*std::sin(0.5*(phi0+psi0));
...
void CuerpoRigido::Rote(double dt){
  ...
  //Actualiza omega apartir del torque(ecuaciones de euler)
  wx+=dt*((Nx/I1)+wyold*wzold*(I2-I3)/I1);
  wy+=dt*((Ny/I2)+wxold*wzold*(I3-I1)/I2);
  wz+=dt*((Nx/I1)+wyold*wyold*(I1-I2)/I3);
}

void CuerpoRigido::CalcularXYZ(double &x, double &y, double &z){
  double xi=x,yi=y,zi=z;
  // Se hace la transformacion del sistema inercial al no inercial
...
int main(void){
  ...
  for(t=0;t<=0.5;t+=deltat){
    std::cout<<t<<" "<<trompo.getTheta()*180/M_PI<<std::endl;
